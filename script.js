...
function renderProducts() {
    ...
    Products.forEach(product => {
        ...
        productElement.innerHTML = `
            ...
            ${quantityInCart}
            <span id="quantity${product.id}" class="quantity"></span>
...
function renderCart() {
    ...
    cart.forEach(item => {
        ...
        cartItemElement.innerHTML = `<p>${item.name}</p> <p>${item.quantity} x ${item.price}</p>`;
        cartContainer.appendChild(cartItemElement);
    });
    const totalElement = document.createElement('p');
    totalElement.classList.add('total-price');
    totalElement.textContent = `Total Price: ${calculateTotalPrice()}`;
